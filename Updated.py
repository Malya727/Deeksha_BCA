...
def safe_filename(filepath):
    """
    Returns filename without extension and path, e.g. "/path/file.csv" -> "file"
    """
...
def compare_dataframes(source_df, target_df, src_fname, tgt_fname):
    # Identify common columns regardless of order
    common_cols = list(set(source_df.columns).intersection(set(target_df.columns)))
    if not common_cols:
        console.print("[bold red]No common columns found for comparison![/bold red]")
        return

    console.print(Panel(f"[bold white]Comparing on columns (order agnostic): {common_cols}[/bold white]",
                        title="Comparison Keys", style="bold magenta"))

    # Select only common columns and sort columns to have same order for both dataframes
    ...
    # Use merge with indicator to find differences
    ...
    # Save files with original base names and descriptive suffixes, no extra appended text
