...
def compare_dataframes(source_df, target_df, src_fname, tgt_fname):
    # Normalize column names (strip spaces and lower case) for robust matching without altering originals
    src_norm_cols = source_df.columns.str.strip().str.lower()
    tgt_norm_cols = target_df.columns.str.strip().str.lower()

    console.print(f"Normalized source columns: {list(src_norm_cols)}")
    console.print(f"Normalized target columns: {list(tgt_norm_cols)}")

    common_cols = list(set(src_norm_cols).intersection(set(tgt_norm_cols)))
    ...
    # Create copies with normalized column names to retain original data intact
    source_sub = source_df.copy()
    source_sub.columns = src_norm_cols
    target_sub = target_df.copy()
    target_sub.columns = tgt_norm_cols

    # Select only common columns, sort rows for aligned comparison
    source_sub = source_sub[common_cols].sort_values(by=common_cols).reset_index(drop=True)
    target_sub = target_sub[common_cols].sort_values(by=common_cols).reset_index(drop=True)
